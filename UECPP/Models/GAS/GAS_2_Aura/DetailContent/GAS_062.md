...
- [GAS 062 使用ExecutionCalculations计算伤害](#gas-062-使用executioncalculations计算伤害)
	- [处理关键点](#处理关键点)
	 - [目录](#目录)
		...
		- [`捕获的属性`  第一步：创建一个结构体](#捕获的属性--第一步创建一个结构体)
		- [`捕获的属性`  第二步：为结构体创建构造](#捕获的属性--第二步为结构体创建构造)
		- [`捕获的属性`  第三步：在全局域中创建一个静态的结构体对象实例并返回](#捕获的属性--第三步在全局域中创建一个静态的结构体对象实例并返回)
		- [`捕获的属性`  第四步：使用官方帮助处理的宏 帮助 声明一些新的捕获定义](#捕获的属性--第四步使用官方帮助处理的宏-帮助-声明一些新的捕获定义)
		- [`捕获的属性`  第五步：构造中，把要捕获的属性添加到 `RelevantAttributesToCapture` 数组](#捕获的属性--第五步构造中把要捕获的属性添加到-relevantattributestocapture-数组)捕获属性的五个步骤下面是在-execute-函数中处理计算和应用捕获的属性)
...
---
### `捕获的属性`  第三步：在全局域中创建一个静态的结构体对象实例并返回
...
---
### `捕获的属性`  第四步：使用官方帮助处理的宏 帮助 声明一些新的捕获定义
...
### `捕获的属性`  第五步：构造中，把要捕获的属性添加到 `RelevantAttributesToCapture` 数组
