...
>  ```
>
>- 最后 遍历 重叠的结果数组，通过 `Overlap.GetActor()` 拿到碰撞检测到的 `Actor`![这里是图片](./GAS_087/8.png)
>
>  ```cpp
...
> ![这里是图片](./GAS_087/10.png)

------

### 此时站在火堆旁边，被敌人攻击，崩溃了

> ### 所以静态函数不允许我们检查任何未实现该接口的调用，就是下面的函数(需要问chatgpt)![这里是图片](./GAS_087/11.png)
>
> ### 此时有两种处理方案：
>
> - #### 一种是使用 `cast` 或者 `if` 
