...
##### 使用到了API：`ExecuteIfBound`
  ...
  - 作用是：如果目标身上有绑定的回调，就激活，如果没有就会返回一个false
...
#### 回调中需要使用数据资产，调用我们之前创建的查找指定技能Tag，返回包含该Tag的结构体函数
  ...
  - ![img](https://api2.mubu.com/v3/document_image/25165450_bf0c4982-daec-4fc5-e160-fa2f3d1fdba4.png)
...
##### 从能力标签规范中可以拿到动态能力标签 `Contaner`
  ...
  - \```const FGameplayAbilitySpec& AbilitySpec;AbilitySpec.DynamicAbilityTags```
