...
- [GAS 023 重点！！！(委托绑定/变参函数模板)属性面板UI中绑定委托更新消息](#gas-023-重点委托绑定变参函数模板属性面板ui中绑定委托更新消息)
  - [处理关键点](#处理关键点)
- [目录](#目录)
    - [UAttributeMenuWidgetController 中创建动态委托,创建在蓝图中需要配置DataAsset的变量](#uattributemenuwidgetcontroller-中创建动态委托创建在蓝图中需要配置dataasset的变量)
    - [情况说明:](#情况说明)
      - [每当调用 HUD 的GetAttributeMenuWidgetController函数,就会触发 `BindCallbacksToDependencies()` 事件](#每当调用-hud-的getattributemenuwidgetcontroller函数就会触发-bindcallbackstodependencies-事件)
      - [又因为在蓝图函数库 UAuraAbilitySystemLibrary 中的 静态 函数 `GetAttributeMenuWidgetController()` 调用了 HUD 的 `GetAttributeMenuWidgetController()` 函数](#又因为在蓝图函数库-uauraabilitysystemlibrary-中的-静态-函数-getattributemenuwidgetcontroller-调用了-hud-的-getattributemenuwidgetcontroller-函数)
      - [理论上是每次调用蓝图节点 `GetAttributeMenuWidgetController()` 函数获取 AttributeMenuWidgetController 时,触发 `BindCallbacksToDependencies()` 事件,但是因为 HUD 中只有当变量为nullptr时才会执行一次,所以相当于初始化的绑定](#理论上是每次调用蓝图节点-getattributemenuwidgetcontroller-函数获取-attributemenuwidgetcontroller-时触发-bindcallbackstodependencies-事件但是因为-hud-中只有当变量为nullptr时才会执行一次所以相当于初始化的绑定)
        - [只执行一次](#只执行一次)
    ...
    - [*WBP\_AttributeMenu* 中 给每个属性UI改名](#wbp_attributemenu-中-给每个属性ui改名)
      - [添加调用,修改逻辑](#添加调用修改逻辑)
    - [*WBP\_TextValueButtonRow* 中 检查GameplayTag标签](#wbp_textvaluebuttonrow-中-检查gameplaytag标签)
    - [测试打印结果](#测试打印结果-1)
    - [四种方法！！！现在需要把类型变量与静态函数的函数指针进行绑定(委托绑定/变参函数模板)](#四种方法现在需要把类型变量与静态函数的函数指针进行绑定委托绑定变参函数模板)
      - [1. 乞丐版: 不绑定直接复制粘贴,修改变量名字,加一堆, 复制粘贴 ,代码质量较低](#1-乞丐版-不绑定直接复制粘贴修改变量名字加一堆-复制粘贴-代码质量较低)
        - [太乞丐,我就不用了](#太乞丐我就不用了)
      - [2. 正常版: 退而求其次, 类型变量与委托绑定](#2-正常版-退而求其次-类型变量与委托绑定)
        - [正常版: 此时因为 UAttributeMenuWidgetController 中逻辑是这么写的,所以如果要绑定需要把所有的属性都搞一遍,太麻烦,此时搞一个 正常版的技术思路 ,使用键值对,保存GameplayTag和委托,这样可以根据Tag,调用对应的事件](#正常版-此时因为-uattributemenuwidgetcontroller-中逻辑是这么写的所以如果要绑定需要把所有的属性都搞一遍太麻烦此时搞一个-正常版的技术思路-使用键值对保存gameplaytag和委托这样可以根据tag调用对应的事件)
          - [运行测试](#运行测试)
          - [思路为 :使用键值对TMap,保存GameplayTag和委托,这样可以将GameplayTag和委托事件绑定,根据Tag,调用对应的事件](#思路为-使用键值对tmap保存gameplaytag和委托这样可以将gameplaytag和委托事件绑定根据tag调用对应的事件)
      - [注意事项！！如果不用委托记得移除！！！！！！！](#注意事项如果不用委托记得移除)
        - [如果不用委托记得移除](#如果不用委托记得移除)
      - [3. 高级但纯纯炫技傻缺版:使用变参函数模板 ————但是可以改良 嘎嘎高级版](#3-高级但纯纯炫技傻缺版使用变参函数模板-但是可以改良-嘎嘎高级版)
        - [原理](#原理)
        - [构造中传的第一个参数tag不对！！！](#构造中传的第一个参数tag不对)
        - [上图大意了](#上图大意了)
        - [测试结果](#测试结果)
        - [起别名,弥补傻缺感](#起别名弥补傻缺感)
        - [使用类模板, 嘎嘎高级版](#使用类模板-嘎嘎高级版)
          - [使用非常方便](#使用非常方便)
      - [4. 高级版:使用函数指针](#4-高级版使用函数指针)
        - [原理](#原理-1)
          - [其实之前写了函数指针类型](#其实之前写了函数指针类型)
          - [演示](#演示)
        - [测试结果](#测试结果-1)
    - [加入TMap中加入主要属性和次要属性](#加入tmap中加入主要属性和次要属性)
    - [测试结果](#测试结果-2)
    - [UAttributeMenuWidgetController 中,现在需要使用ASC的当属性变化广播绑定委托API,绑定所有属性回调使用lambda](#uattributemenuwidgetcontroller-中现在需要使用asc的当属性变化广播绑定委托api绑定所有属性回调使用lambda)
...
#### 3. 高级但纯纯炫技傻缺版:使用变参函数模板 ————但是可以改良 嘎嘎高级版
___________________________________________________________________________________________
