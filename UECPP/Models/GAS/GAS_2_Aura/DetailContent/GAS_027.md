...
# GAS 027 使用数据资产将 输入 与 输入触发的Tag 绑定

___________________________________________________________________________________________

# 目录
- [GAS 027 使用数据资产将 输入 与 输入触发的Tag 绑定](#gas-027-使用数据资产将-输入-与-输入触发的tag-绑定)
- [目录](#目录)
    - [视频链接:](#视频链接)
    - [创建DataAsset资产](#创建dataasset资产)
    - [FAuraGameplayTags中添加按键的Tag](#fauragameplaytags中添加按键的tag)
    - [此时运行项目 项目设置中查看GmeplayTag,若有标签说明操作没有问题](#此时运行项目-项目设置中查看gmeplaytag若有标签说明操作没有问题)
    - [创建蓝图类数据资产继承自上面创建的c++类UAuraInputConfig取名DA\_AuraInputConfig](#创建蓝图类数据资产继承自上面创建的c类uaurainputconfig取名da_aurainputconfig)
    - [创建对应的InputAction](#创建对应的inputaction)
    - [IMC\_AuraContext中配置按键输入](#imc_auracontext中配置按键输入)
    - [DA\_AuraInputConfig中配置 FInputAction和FGameplaTag](#da_aurainputconfig中配置-finputaction和fgameplatag)
