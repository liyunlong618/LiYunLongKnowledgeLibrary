...
### [跳转 添加两个新敌人 相关](#下面是添加两个新敌人-相关)
### [跳转 GameplayCue 相关](#下面是GameplayCue-相关)
...
### [GAS 134 重构函数参数的思路方法！！！初始化和获取 `SpellMenuWidgetController`](./DetailContent/GAS_134.md)

### [GAS 135 装备技能区的显示技能的逻辑](./DetailContent/GAS_135.md)

### [GAS 136 创建技能状态和类型的Tag标记技能，并在(初始化) `GiveAbility` 时赋予 `DynamicAbilityTags` 的Tag；根据技能状态Tag，在UI中使用不同显示策略](./DetailContent/GAS_136.md)

### [GAS 137服务器中在角色升级时遍历技能，找到合格的技能，然后学习（在服务器），并同步给客户端UI](./DetailContent/GAS_137.md)

### [GAS 138 初始化时和升级时同步技能点；处理选择技能树中同时只能选中一个技能的逻辑](./DetailContent/GAS_138.md)

### [GAS 139 使用广播本地根据技能状态和技能点处理花费技能点按钮和装备技能按钮的启用和禁用](./DetailContent/GAS_139.md)

### [GAS 140 在角色升级时，将需要的信息同步到UI；处理当一个广播的内容依赖于两个不一定先后触发的回调时的处理方案](./DetailContent/GAS_140.md)

### [GAS 141 在服务器处理消费技能点升级技能等级和解锁技能，立即Mark同步一次，然后使用Client将结果同步到客户端](./DetailContent/GAS_141.md)

### [GAS 142 了解富文本 `RichText` 的使用步骤](./DetailContent/GAS_142.md)

### [GAS 143 C++中 使用 `Printf` 格式化字符串输出到富文本；每个GA中都需要一个返回自身技能描述的虚函数；GA中拿到CD和技能消耗的方法](./DetailContent/GAS_143.md)

### [GAS 144 记录选中技能树中的技能球，处理点击和取消的逻辑；处理技能详情显示升级时的bug](./DetailContent/GAS_144.md)

### [GAS 145 制作装备技能点击提示动画和相关显示和隐藏的逻辑；显示后动画循环](./DetailContent/GAS_145.md)

### [GAS 146 点击装备技能的逻辑，(需要记录之前的技能球，移除InputTag，旧的UI还原，新的添加 `InputTag`，两个都需要更新状态Tag)；需要 `RPC` 上 `服务器` 处理，然后同步给 `客户端`](./DetailContent/GAS_146.md)

### [GAS 147 处理装备技能广播技能新位置后，其他技能球贴图材质清空的bug；同步装备技能 `InputTag` 变化到主UI(WBP_Overlay)；装备技能后取消选择](./DetailContent/GAS_147.md)

### [GAS 148 处理重复开关菜单后，(音效等)效果叠加的bug](./DetailContent/GAS_148.md)
...
### [GAS 149 重构属性伤害Tag和属性伤害的映射逻辑，添加DebuffTag和映射](./DetailContent/GAS_149.md)

### [GAS 150 GA中添加Debuff参数；创建结构体保存传递要使用的诸多参数；GA中使用上下文设置结构体参数；使用结构体中的信息对目标应用GE](./DetailContent/GAS_150.md)

### [GAS 151 重构让火球携带初始化参数后的结构体，并在重叠时，设置结构体中关键参数，最后使用结构体应用GE完成逻辑；使用Rider快捷抽象函数](./DetailContent/GAS_151.md)

### [GAS 152 计算准备应用Debuff；为敌人添加属性伤害抗性；处理未能捕获属性伤害抗性的BUG](./DetailContent/GAS_152.md)

### [GAS 153 FGameplayContext中携带结构体；在AS中读取Context中的参数，准备应用Debuff](./DetailContent/GAS_153.md)

### [GAS 154 在C++中创建并应用Debuff的GE流程（期间读取Context中的配置设置GE参数）；处理敌人死亡时不应用Debuff](./DetailContent/GAS_154.md)

### [GAS 155 自建 NS组件 监听 DebuffTag](./DetailContent/GAS_155.md)

### [GAS 156 敌人死亡时（Mesh开启模拟物理后）将Mesh沿火球方向击飞（结构体和 `Context` 携带击飞方向）；使用GA的Tag打断制作：Debuff不会触发受击GA](./DetailContent/GAS_156.md)

### [GAS 157 为Aura和敌人创建攻击时击飞的攻击特效；为角色添加滞空动画；场景增加空气墙](./DetailContent/GAS_157.md)
...
### [GAS 158 在火球子类上重构生成火球逻辑；设计根据技能范围和火球数量计算生成角度](./DetailContent/GAS_158.md)

### [GAS 159 计算部分抽象静态函数；为火球添加重力；使用抛射物组件的追踪导航组件](./DetailContent/GAS_159.md)

### [GAS 160 制作点击地面移动时，目标地点的NS特效](./DetailContent/GAS_160.md)

### [GAS 161 GA中使用GAS异步节点  `WaitInputPress` 等 监听输入按键状态（需要在源码中将事件和按键同步服务器）](./DetailContent/GAS_161.md)

### [GAS 162 创建闪电技能C++类；创建并使用释放技能蒙太奇；处理释放持续引导类技能时，禁用角色移动](./DetailContent/GAS_162.md)

### [GAS 163 创建阻挡按键Tag在释放技能阶段阻止某些按键状态](./DetailContent/GAS_163.md)

### [GAS 164 创建 `cue Static` 项目中索引优化cue的路径，在心情文件中并配置；参考lyra在配置文件中修改，RPC最大通信数量为10](./DetailContent/GAS_164.md)

### [GAS 165 创建和使用 `GameplayCueNotify_Actor` 生成绑定特效和音效；给NS传参](./DetailContent/GAS_165.md)

### [GAS 166 修复未开启属性复制导致的画面表现不同步；为技能添加Loop声音](./DetailContent/GAS_166.md)

### [GAS 167 创建并使用自定义碰撞通道，修复攻击目标的穿透问题](./DetailContent/GAS_167.md)
...
### [GAS 168 处理闪电技能链式伤害的首个目标，射线检测阻挡的目标，播放声音，释放技能后取消目标选择](./DetailContent/GAS_168.md)

### [GAS 169 处理闪电技能链式伤害的后续添加目标逻辑，获取首个目标附近最近的N个单位](./DetailContent/GAS_169.md)

### [GAS 170 制作闪电链视觉效果，为目标配置Cue的Param，结束技能后移除](./DetailContent/GAS_170.md)

### [GAS 171 使用Timer为闪电链制作掉血逻辑，并在首个目标死亡后结束技能](./DetailContent/GAS_171.md)

### [GAS 172 使用监听处理闪电链技能释放期间首个目标和其他目标死亡的逻辑，添加判断闪电技能最短触发时间的逻辑](./DetailContent/GAS_172.md)

### [GAS 173 修复火球击中死亡敌人时漂浮的bug](./DetailContent/GAS_173.md)

### [GAS 174 制作眩晕debuff逻辑；处理技能之间的打断；处理DS模式下客户端的bug；需要注意tag不会复制！！！](./DetailContent/GAS_174.md)

### [GAS 175 客户端不能 `EndAbility` ！火球Actor除了开启复制，还需要开启复制运动！添加眩晕特效；保证玩家也能被眩晕和DS客户端修改逻辑](./DetailContent/GAS_175.md)

### [GAS 176 使用属性复制，为闪电技能添加敌人的响应动画逻辑；优化：避免频繁触发不必要的过渡蒙太奇和GA](./DetailContent/GAS_176.md)
...
##### 处理关键点
1. ###### `Printf` 中使用 `\n` 换行
2. ###### `Printf` 中使用 `%.f` 不看小数点
3. ###### `Printf` 中使用 `%.1f` 小数点后保留一位
...
2. ###### 使用 `while` 循环 处理添加最近目标的逻辑思路（防止无限循环！）
